...
Kullanim:
    ...
    python scripts/seed_demo_data.py --with-erp-connectors

Ortam degiskeni ile veritabani baglantisini ozellestirebilirsin:
    DATABASE_URL = "postgresql://erpilot:erpilot@localhost:5433/erpilot"
...
# DATABASE_URL ortam değişkenini alıyoruz
RAW_DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql://erpilot:erpilot@postgres:5432/erpilot"
)

# psycopg2 için SQLAlchemy asyncpg vb. sürücü eklerini temizliyoruz
# 'postgresql+asyncpg://...' -> 'postgresql://...'
if "+asyncpg" in RAW_DATABASE_URL:
    DATABASE_URL = RAW_DATABASE_URL.replace("+asyncpg", "")
elif "+psycopg2" in RAW_DATABASE_URL:
    DATABASE_URL = RAW_DATABASE_URL.replace("+psycopg2", "")
else:
...
def seed_erp_connectors(cur, tenant_id):
    """
    TASK-035c
    ERPNext ve Dolibarr baglantilarini demo amacli ekler.
    """
    connectors = [
        (
            str(uuid.uuid4()),
            ...
            "erpnext",
            json.dumps({
                "base_url": os.getenv("ERPNEXT_URL", "http://erpnext:8080"),
                "api_key": os.getenv("ERPNEXT_API_KEY", "changeme"),
                "api_secret": os.getenv("ERPNEXT_API_SECRET", "changeme"),
            }),
        ),
        (
            str(uuid.uuid4()),
            ...
            "dolibarr",
            json.dumps({
                "base_url": os.getenv("DOLIBARR_URL", "http://dolibarr:80"),
                "api_key": os.getenv("DOLIBARR_API_KEY", "changeme"),
            }),
        ),
    ]

    inserted = 0
    for conn_id, t_id, name, connector_type, config_json in connectors:
        cur.execute(
            "SELECT id FROM erp_connections WHERE tenant_id = %s AND name = %s LIMIT 1",
            (t_id, name),
        )
        if cur.fetchone():
            continue
        cur.execute(
            ...
            """,
            (conn_id, t_id, name, connector_type, config_json),
        )
        inserted += 1
...
def main():
    ...
    parser.add_argument(
        "--with-erp-connectors",
        action="store_true",
        help="ERPNext ve Dolibarr bağlantılarını da oluştur"
    )
    ...
    try:
        with conn.cursor() as cur:
            ...
            if args.with_erp_connectors if hasattr(args, 'with_erp_connectors') else False:
                pass

            if args.with_erp_connectors:
                n_conn = seed_erp_connectors(cur, tenant_id)
                print(f"erp_connections: {n_conn} yeni bağlantı eklendi")
